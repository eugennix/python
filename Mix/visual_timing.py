...
def compare(fs, args):
    ...
    for f in fs:
        plt.plot(xs, [timed(f, chunk) for chunk in args],
                 label=f.__name__)
    plt.title('Минимальное время работы функции')
    plt.legend()
    plt.grid(True)
    plt.show()
...
def compareAll(fs, args):
    ...
    iteration = 10
    for f in fs:
        plt.plot(xs, [timed_CountedAllCycles(f, chunk, n_iter=iteration) for chunk in args],
                 label=f.__name__)
    plt.title(f'Общее время работы функции за {iteration} иттераций')
    plt.legend()
    plt.grid(True)
    plt.show()
